...
const Item = ({ data, setModalVisible, selectedItem, numberCode, selectBukrs }) => {
    const toggleModal = (item) => {
        setModalVisible(false);
        if (selectBukrs) {
            console.log('selected')
            selectedItem(item.name.substring(0, numberCode));
        }
        else {
            selectedItem({
                code: item.name.substring(0, numberCode),
                text: item.name.substring(numberCode + 3)
            })
        }
    };
    return (
